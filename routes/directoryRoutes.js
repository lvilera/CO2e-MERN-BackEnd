...
router.delete('/clear-all', requireAdmin, async (req, res) => {
  try {
    // const userResult = await UserDirectory.deleteMany({}); // REMOVED: Protect user data
...
// DELETE: Clear only admin directory listings (safer alternative to clear-all)
router.delete('/clear-admin-only', requireAdmin, async (req, res) => {
  try {
    const adminResult = await AdminDirectory.deleteMany({});
    ...
    res.json({
      ...
      deletedCount: adminResult.deletedCount,
      note: 'User directory entries were preserved'
    });
  } catch (err) {
...
router.get('/admin/user-submissions', async (req, res) => {
  try {
...
// PUT: Approve/Reject user directory entry
router.put('/admin/user-submissions/:id', async (req, res) => {
  try {
    const { id } = req.params;
...
// DELETE: Delete user directory entry
router.delete('/admin/user-submissions/:id', async (req, res) => {
  try {
    ...
    res.json({
      ...
      deletedEntry: {
        company: deletedEntry.company,
        email: deletedEntry.email
      }
    });
  } catch (err) {
